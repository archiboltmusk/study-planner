...
const PHASES = [
  { id: "blitz",    label: "Blitz Pass",          days: [1,  18] as [number,number], color: "#ff4d4d" },
  { id: "revision", label: "Rapid Revision",       days: [19, 24] as [number,number], color: "#ffaa00" },
  { id: "mock",     label: "Mock & Consolidate",   days: [25, 28] as [number,number], color: "#00e5cc" },
];
...
const SUBJECT_COLORS: Record<string,string> = {
  Medicine: "#ef4444", Surgery: "#a855f7", Pathology: "#f97316",
  Pharmacology: "#10b981", OBG: "#ec4899", Paediatrics: "#38bdf8",
  PSM: "#8b5cf6", Microbiology: "#eab308", Biochemistry: "#06d6a0",
  Forensic: "#94a3b8", "Anatomy/Physio": "#64748b",
  Revision: "#f59e0b", "Full Mock": "#06b6d4", "Exam Eve": "#00e5cc",
};
  ...
  return (
    <div className="space-y-6">
      ...
      <motion.div {...cardAnim} transition={{delay:0.0}}
        className="glass rounded-2xl p-5 border-l-4 border-l-[#ff4d4d] relative overflow-hidden">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label:"Days Done",   value:`${completedDays.length}/28`, icon:<CheckCircle className="w-5 h-5"/>, color:"#ff4d4d",  sub:`${Math.round((completedDays.length/28)*100)}% complete` },
          { label:"Best Streak", value:`${streak}`,                  icon:<Flame className="w-5 h-5"/>,       color:"#ffaa00",  sub:streak>=3 ? "On fire! 🔥" : "Build your streak" },
...
function TimerView() {
  ...
  const color = mode === "break" ? "#10b981" : "#ff4d4d";
      ...
      <div className="flex items-center gap-2">
        {Array.from({length:4}).map((_,i) => (
          <div key={i} className="w-3 h-3 rounded-full transition-all"
            style={{background: i < sessions%4 ? "#ff4d4d" : "rgba(255,255,255,0.1)"}} />
        ))}
      ...
      <header className="sticky top-0 z-40 glass border-b border-white/6">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 shrink-0">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center glow-red" style={{background:"#ff4d4d"}}>
        ...
        <div className="h-0.5 bg-white/4">
          <div className="h-full transition-all duration-700" style={{width:`${progress}%`, background:"linear-gradient(90deg,#ff4d4d,#ffaa00,#00e5cc)"}} />
        </div>
